      ...
      <Draggable key={draggableId} draggableId={draggableId} index={index}>
        {(provided, snapshot) => (
            <>
              ...
              {/*<Droppable key={"DroppableTreeItem-" + id} droppableId={"DroppableTreeItem-" + id}>
                {(provided, snapshot) => (
                    <div ref={provided.innerRef} {...provided.droppableProps}>
                      {"DroppableTreeItem-" + id}
                    </div>
                )}
              </Droppable>*/}
            </>
...
const DraggableTreeView__bak2 = ({treeItems, onDragEnd}) => {
            ...
            {(provided, snapshot) => (
                <ul ref={provided.innerRef} {...provided.droppableProps}
                    style={{
                      ...
                      border: snapshot.isDraggingOver ? '2px solid blue' : 'none',
                    }}
                >
      ...
      <SimpleTreeView>
        <DragDropContext onDragEnd={(result) => onDragEnd(result, treeItems)}>
          <>
            {treeItems.map((item, index) => {
              const id = item.id;
              const draggableId = `DraggableTreeItem-${item.id}`;
              return (
                  <Droppable key={"DroppableTreeItem-" + id} droppableId={"DroppableTreeItem-" + id}>
                    {(provided, snapshot) => (
                        <div ref={provided.innerRef} {...provided.droppableProps}
