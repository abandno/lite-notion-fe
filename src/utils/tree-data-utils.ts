// @ts-nocheck
// Copy and enhance from `react-sortable-tree`
 ...
 */
export const getNodeKeyFn = ({node, treeIndex=undefined}) => node.id
...
export const changeNodeAtPath = ({
  ...
  const traverse = ({
    ...
    if (pathIndex >= path.length - 1) { // 顺藤摸瓜摸到头了
      // If this is the final location in the path, return its changed form
      return typeof newNode === 'function'
          ? newNode({node, treeIndex: currentTreeIndex})
          : newNode
    }
    if (!node.children) { // 顺藤摸瓜, 摸到中途藤断了
  ...
  try {
    changeNodeAtPath({
      ...
      newNode: ({node, treeIndex}: GetTreeItemChildren) => {
        // 复用 changeNodeAtPath 的逻辑, 调用 newNode 函数的时机即在目标节点上
...
export const getTreeFromFlatData = ({
  ...
  const trav = (parent, path = []) => { // 增加 path 入参 --L&J
    ...
    if (parentKey in childrenToParents) {
      return {
        ...parent,
        path: [...path],
        children: childrenToParents[parentKey].map((child) => trav(child, [...path])),
        root: path[0],
      };
    }
    return {...parent, path: [...path], root: path[0]};
  };
...
/**
 * 根据 node key 查找的快捷方法
 * 相对 find by path 开销更大
 * @author L&J
 * @since 2024-6-14 00:56:44
 */
export const findByKey = ({key, treeData, getNodeKey}) => {
  let targetNode = null;
  walk({
    treeData,
    getNodeKey,
    ...
    callback: ({
      ...
      if (getNodeKey({node, treeIndex}) == key) {
        targetNode = node;
        return false; // 快速结束
      }
      return true;
...
export const getSpaceName = (node) => {
